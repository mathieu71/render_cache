...
namespace Drupal\render_cache\StackMiddleware;
...
class PageCache implements HttpKernelInterface {
  ...
  protected function lookup(Request $request, $type = self::MASTER_REQUEST, $catch = TRUE) {
    ...
    if ($max_age !== Cache::PERMANENT && $response->getLastModified() !== NULL ) {
//         $age = REQUEST_TIME - $response->getLastModified()->getTimestamp();
//         $response->setMaxAge($max_age - $age);
        $response->headers->removeCacheControlDirective('max-age');
        $dt = new \DateTime();
        $dt->setTimestamp($response->getLastModified()->getTimestamp() + $max_age);
    ...
    if ($last_modified) {
      ...
      if ( // $if_modified_since && $if_none_match
        // etag must match.
        ($if_none_match == $response->getEtag() || $if_none_match == 'W/'.$response->getEtag() )
        ...
        foreach (array_keys($response->headers->all()) as $name) {
          if (!in_array($name, ['etag', 'content-location', 'expires', 'cache-control', 'vary'])) {
  ...
  protected function storeResponse(Request $request, Response $response) {
    ...
    if ($response->isClientError()) {
      ...
      if ($cache_ttl_4xx > 0) {
        ...
        $expire = $response->getCacheableMetadata()->getCacheMaxAge();
    }
    if ($expire !== 0) {
